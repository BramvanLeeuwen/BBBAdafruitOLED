...
class BBB_OLED : public Adafruit_GFX 
{
 public:
  BBB_OLED();
  BBB_OLED(int i2cbus, int i2caddress);
  ~BBB_OLED();
  // SPI Init
  //boolean init(int8_t DC, int8_t RST, int8_t CS, uint8_t OLED_TYPE);
  
  // I2C Init
  boolean init(int8_t resetpin);

  boolean oled_is_spi_proto(uint8_t oledtype); /* to know protocol before init */
  boolean select_oled(uint8_t oledtype) ;
  ...
  private:
  I2C   *i2c;  //The I2C2 Bus on the Beaglebone op DDA P9_20 and SCL P9_19
  GPIO  *resetpin;
