 ...
 /* if (oled_type == OLED_SEEED_I2C_96x96 )
  {
    ...
    // Get old value to not touch the other nibble
    c = *p;
    
    // We are on High nibble ?
    if ( ((y/2)&1) == 1 )
    {
      c &= 0x0F;
...
//void BBB_OLED::setGrayLevel(uint8_t grayLevel)
//{
...
/*void BBB_OLED::putSeedChar(char C)
{
...
void BBB_OLED::putSeedString(const char *String)
{
    unsigned char i=0;
    while(String[i])
    {
